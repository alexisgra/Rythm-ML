...
class NoteSequenceDrawing {
    constructor(sequence, canvas) {
        this.config = {
            noteHeight: 40,
            noteSpacing: 0.05,
            pixelsPerTimeStep: 100,  // The bigger this number the "wider" a note looks,
            minPitch: 100,
            maxPitch: 1
        }
    ...
    drawSequence(currentNote) {
        ...
        for (let i = 0; i < this.noteSequence.notes.length; i++) {
            ...
            const w = (note.endTime - note.startTime) * this.config.pixelsPerTimeStep * 10;
            ...
            if (this.isPaintingCurrentNote(note, currentNote)) {
                this.ctx.fillStyle=`rgba(240, 84, 119)`;  // pink
            } else if (i <= this.primerNotes) {
                this.ctx.fillStyle=`rgba(111, 201, 198)`;  // teal
            }
            else {
                this.ctx.fillStyle=`rgba(8, 41, 64)`;  // dark blue
            }
